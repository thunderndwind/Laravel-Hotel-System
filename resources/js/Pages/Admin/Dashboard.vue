      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div v-for="(stat, key) in stats" :key="key" 
            ...
            <div class="text-gray-500 text-sm">{{ formatLabel(key) }}</div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
            <div class="flex justify-between items-center mb-6">
              <h2 class="text-2xl font-semibold text-gray-900">Manage Receptionists</h2>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="receptionist in receptionists" :key="receptionist.id">
                    <td class="px-6 py-4 whitespace-nowrap">{{ receptionist.user ? receptionist.user.name : '' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ receptionist.user ? receptionist.user.email : '' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span :class="[
                        'px-2 inline-flex text-xs leading-5 font-semibold rounded-full',
                        receptionist.user ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                      ]">
                        {{ receptionist.user ? 'Active' : 'Pending' }}
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                      <div class="flex items-center space-x-3">
                        <button @click="editReceptionist(receptionist)" 
      ...
      <div class="p-6">
        ...
        <form @submit.prevent="submitForm" class="mt-6 space-y-6">
          <div>
            ...
            <Input id="name" v-model="form.name" type="text" required class="mt-1 block w-full" />
          ...
          <div>
            ...
            <Input id="national_id" v-model="form.national_id" type="text" required class="mt-1 block w-full" />
            <InputError :message="form.errors.national_id" class="mt-2" />
          </div>

          <div>
            <Label for="phone_number">Phone Number</Label>
            <Input id="phone_number" v-model="form.phone_number" type="text" required class="mt-1 block w-full" />
