<template>
        ...
        <div class="container">
          <div class="row justify-content-end mb-50">
            <div class="col-12 col-md-4 text-right" v-if="can.create">
              <router-link :to="{ name: 'clients.create' }" class="btn palatin-btn">
                Add New Client
              </router-link>
            </div>
          </div>
                        ...
                        <tr v-for="client in clients.data" :key="client.id">
                          ...
                          <td>
                            <div class="d-flex align-items-center">
                              <img :src="client.avatar_image ? '/storage/' + client.avatar_image : '/img/core-img/default-avatar.png'" 
                          ...
                          <td>
                            <span :class="['badge', client.approved_at ? 'badge-success' : 'badge-warning']">
                              {{ client.approved_at ? 'Approved' : 'Pending' }}
                            </span>
                          </td>
                          <td>
                            ...
                            <button v-if="can.approve && !client.approved_at" 
                        ...
                        <nav aria-label="Page navigation">
                          <ul class="pagination">
                            <li class="page-item" :class="{ disabled: !clients.prev_page_url }">
                              <a class="page-link" href="#" @click.prevent="changePage(clients.current_page - 1)">
                                Previous
                              </a>
                            </li>
                            <li class="page-item" v-for="page in clients.last_page" 
                                :key="page" :class="{ active: page === clients.current_page }">
  ...
  const approveClient = (client) => {
    if (confirm('Are you sure you want to approve this client?')) {
      router.post(route('clients.approve', client.id), {}, {
        preserveScroll: true,
        onSuccess: () => {
  ...
  const changePage = (page) => {
    router.get(route('clients.index'), { page }, {
      preserveScroll: true,
      preserveState: true
    });
  };
